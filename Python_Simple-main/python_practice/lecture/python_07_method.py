...
# - 함수에 전달되는 입력값(input)
# - 함수에 정의문과 호출문의 parameter 갯수가 동일해야함
...
# - return 이 없는 경우 들여쓰기 종료 함수 종료로 간주
# - return 문 다음에 오는 코드는 실행 안됌(Error x)
def soju_yn(age):
    if age>= 20:
        return 1 # 구매가능
    else:
...
# - 함수내의 변수는 자시닝 속한 코드 블록이 종료되면 소멸
